...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const application = await Application.findById(interaction.customId.replace('app_accept_', '')).catch(() => null);
    ...
    if (user) {
      await user.send(`🎉 Congratulations! Your **${panel?.name || 'staff'}** application in **${interaction.guild.name}** was **accepted**.`).catch(() => {});
    }

    await interaction.update({ components: [] });
    await interaction.followUp({ embeds: [embeds.success(`Application accepted by ${interaction.user}.`)] });
  }
};
